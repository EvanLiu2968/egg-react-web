...
(function(console) {
  ...
  console.image = function(url, scale) {
    ...
    img.onload = function() {
      ...
      var dim = getBox(this.width * scale, 120);
      console.log("%c" + dim.string, dim.style + "background: url(" + url + "); background-repeat: no-repeat; background-size: " + (this.width * scale) + "px " + (this.height * scale) + "px; color: transparent;");
    };

    img.src = url;
  };
  console.draw = function(upper, lower, width, height) {
    var canvas = document.createElement("canvas"),
      ctx = canvas.getContext("2d"),
      width = width || 500,
      height = width || 500,
